...
const SectionD20 = ({ myRef, IsVisible }) => {
  //SCROLLING
 ...
 la siguiente linea reemplaza a todo esto...*/
  ...
  return (
    <section className="text-white pt-10 px-4 md:px-10 py-10 w-full bg-hero2 bg-cover bg-fixed mt-16">
      <h2 ref={myRef} className={`${IsVisible ? "animate-left" : ""} text-3xl`}>
        ...
        <Link to="/maquinas">
          <div className="m-0 md:m-12">
            <img
              src="/d20.png"
              className={`${
                IsVisible ? "animate-left" : ""
              } mx-auto border-2 m-4 rounded-xl border-black w-full`}
            />
          </div>
        </Link>
        <div
          className={`${IsVisible ? "animate-left" : ""} my-auto p-4 space-y-2`}
        >
          <p>
